...
def cluster_transformers(transformers,
                         by="efficiency_el",
                         share_clusters=0.1):
    ...
    created for plants of the same fuel and the same (CHP) operation mode.
    ...
    for fuel, mode in zip(x1.flatten(), x2.flatten()):
        n = math.ceil(
            transformers.loc[(transformers["from"] == fuel)
                             & (transformers["mode"] == mode)].shape[0]
            * share_clusters)

        # Introduce a limitation for cluster numbers
        if n > transformers.loc[(transformers["from"] == fuel)
                                & (transformers[
                                       "mode"] == mode), by].nunique():
            n = transformers.loc[
                (transformers["from"] == fuel)
                & (transformers["mode"] == mode), by].nunique()

        # Exclude the option of zero power plants to be clustered
        if not transformers["cluster"].loc[
            (transformers["from"] == fuel)
            & (transformers["mode"] == mode)].empty:
            # Do the actual clustering
            transformers.loc[(transformers["from"] == fuel)
                             & (transformers["mode"] == mode), "cluster"] = (
                    KMeans(n_clusters=n).fit(transformers.loc[
                        (transformers["from"] == fuel)
                        & (transformers[
...
def group_power_plants(transformers, grouping_cols,
                       mean_cols, sum_cols, other_cols):
    ...
    tf_grouped_mean = transformers[mean_cols + grouping_cols].groupby(
        grouping_cols).aggregate(wm)
    tf_grouped_sum = transformers[sum_cols + grouping_cols].groupby(
        grouping_cols).sum()
    tf_grouped_other = transformers[other_cols + grouping_cols].groupby(
        grouping_cols).nth(0)
